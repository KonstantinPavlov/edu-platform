...
export class NewsService {
    ...
    private resourceSearchUrl = SERVER_API_URL + 'api/_search/news';
    ...
    search(req?: any): Observable<HttpResponse<News[]>> {
        const options = createRequestOption(req);
        return this.http.get<News[]>(this.resourceSearchUrl, { params: options, observe: 'response' })
            .map((res: HttpResponse<News[]>) => this.convertArrayResponse(res));
    }
    ...
    private convertItemFromServer(news: News): News {
        ...
        copy.newsDate = this.dateUtils
            .convertDateTimeFromServer(news.newsDate);
        return copy;
    }
    ...
    private convert(news: News): News {
        ...
        copy.newsDate = this.dateUtils.toDate(news.newsDate);
        return copy;
    }
}
