...
public class CourseResource {
    ...
    @GetMapping("/courses")
    @Timed
    public ResponseEntity<List<CourseDTO>> getAllCourses(Pageable pageable, @RequestParam(required = false) String filter) {
        if ("payments-is-null".equals(filter)) {
            log.debug("REST request to get all Courses where payments is null");
            return new ResponseEntity<>(courseService.findAllWherePaymentsIsNull(),
                    HttpStatus.OK);
        }
